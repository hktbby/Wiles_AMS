...
$paginationHtml = ''; // Initialize empty variable for pagination HTML
...
try {
    ...
    $totalQuery = $connection->prepare("SELECT COUNT(*) AS total FROM tenant");
    $totalQuery->execute();
    $totalRecords = $totalQuery->fetch(PDO::FETCH_ASSOC)['total'];
    $totalPages = ceil($totalRecords / $recordsPerPage);

    // Fetch users for the current page
    $query = $connection->prepare("SELECT tenant.tenantID, tenant.firstname, tenant.lastname, tenant.payment, tenant.sex, tenant.Username, tenant.user_profile_picture, CONCAT(tenant_add_numBed, ', ', tenant_Amount) AS address FROM tenant INNER JOIN tenant_info ON tenant.tenantID = tenant_info.tenantID LIMIT :offset, :recordsPerPage");
